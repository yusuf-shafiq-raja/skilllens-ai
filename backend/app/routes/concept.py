...
@router.get("/skill/{skill_id}", response_model=list[ConceptResponse])
def get_skill_concepts(
    skill_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    return get_concepts_by_skill(db, skill_id, current_user)
...
def get_concept(
    ...
    concept = get_concept_by_id(db, concept_id, current_user)
...
def edit_concept(
    ...
    try:
        return update_concept(db, concept_id, concept, current_user)
    except ValueError as e:
...
def remove_concept(
    ...
    try:
        return delete_concept(db, concept_id, current_user)
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
